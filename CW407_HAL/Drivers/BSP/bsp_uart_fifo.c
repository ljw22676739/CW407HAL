...
*********************************************************************************************************
*	函 数 名: bsp_InitUart
*	功能说明: 初始化串口,并对全局变量赋初值.硬件交由CubeMX生成的代码初始化
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
...
*********************************************************************************************************
*	函 数 名: comSendString
*	功能说明: 向串口发送一串字符。数据放到发送缓冲区后立即返回，由中断服务程序在后台完成发送
*	形    参: _ucPort: 端口号(COM1 - COM6)
*			  _ucaBuf: 待发送的字符串
*	返 回 值: 无
*********************************************************************************************************
*/
void comSendString(COM_PORT_E _ucPort, char *_ucaBuf)
{
	UART_T *pUart;
	...
	UartSend(pUart, (uint8_t*)_ucaBuf, strlen((const char*)_ucaBuf));
}
...
{
#if UART1_FIFO_EN == 1
	g_tUart1.hUart = &huart1;						/* STM32 串口设备 */
...
#if UART2_FIFO_EN == 1
	g_tUart2.hUart = &huart2;						/* STM32 串口设备 */
...
#if UART3_FIFO_EN == 1
	g_tUart3.hUart = &huart3;						/* STM32 串口设备 */
...
#if UART4_FIFO_EN == 1
	g_tUart4.hUart = &huart4;						/* STM32 串口设备 */
...
#if UART5_FIFO_EN == 1
	g_tUart5.hUart = &huart5;						/* STM32 串口设备 */
...
static void UartSend(UART_T *_pUart, uint8_t *_ucaBuf, uint16_t _usLen)
{
    ...
    SET_BIT(_pUart->hUart->Instance->CR1, USART_CR1_TXEIE);
}
...
*********************************************************************************************************
*	函 数 名: UartIRQ
*	功能说明: 供中断服务程序调用，通用串口中断处理函数(缓存区溢出后自动覆盖最前的数据)
*	形    参: _pUart : 串口设备
*	返 回 值: 无
*********************************************************************************************************
*/
static void UartIRQ(UART_T *_pUart)
{
    uint32_t isrflags   = READ_REG(_pUart->hUart->Instance->SR);
...
//          huart1.Instance->DR = _pUart->pTxBuf[_pUart->usTxRead];
...
*********************************************************************************************************
*/
static void GprsUartIRQ(UART_T *_pUart)
{
    uint32_t isrflags   = READ_REG(_pUart->hUart->Instance->SR);
    uint32_t cr1its     = READ_REG(_pUart->hUart->Instance->CR1);
    uint32_t cr3its     = READ_REG(_pUart->hUart->Instance->CR3);
	...
	if (((isrflags & USART_SR_RXNE) != RESET) && ((cr1its & USART_CR1_RXNEIE) != RESET))
	{
		...
		//存入另一个较小的存储区,用于检查GPRS的命令回复
        if (GPRS_RxBufTmp_cnt < GPRS_RX_BUF_TMP_LEN)
        {
            GPRS_RxBufTmp[GPRS_RxBufTmp_cnt++] = ch;
        }
