...
*       V2.0    2016-10-15 Devin   移植到HAL库和CubeMX配合,暂不考虑和其他器件共享spi,同时不使用文件bsp_spi_bus.c,
*                                  独立结构。bsp_spi_bus.c也仅适用于单个spi总线的情况。暂时不考虑软件SPI
*
...
/* ****************************************************************************
   需要移植的接口函数
* ****************************************************************************/
#define SPI_HANDLE &hspi1           //所用的SPI接口的句柄
/* 串行Flsh的片选GPIO端口  */
#define SF_CS_0()			HAL_GPIO_WritePin(GPIOI, GPIO_PIN_8, GPIO_PIN_RESET)
#define SF_CS_1()			HAL_GPIO_WritePin(GPIOI, GPIO_PIN_8, GPIO_PIN_SET)
 
static uint8_t bsp_spiRead0(void)
{
    uint8_t data;
    HAL_SPI_TransmitReceive(SPI_HANDLE,&data,&data,1,0xffff);
    return data;
}

static uint8_t bsp_spiRead1(void)
{
    uint8_t data;
    HAL_SPI_TransmitReceive(SPI_HANDLE,&data,&data,1,0xffff);
    return data;
...
static void bsp_spiWrite0(uint8_t _TxData)
{
    uint8_t Rxdata;
    HAL_SPI_TransmitReceive(SPI_HANDLE,&_TxData,&Rxdata,1,0xffff);
}

static uint8_t bsp_spiWrite1(uint8_t _TxData)
{
    uint8_t Rxdata;
    HAL_SPI_TransmitReceive(SPI_HANDLE,&_TxData,&Rxdata,1,0xffff);
    return Rxdata;
...
*********************************************************************************************************
*/
static void sf_ConfigGPIO(void)//GPIO配置由CubeMX统一配置
{
//  /*
...
*********************************************************************************************************
*/
static void sf_CfgSpiHard(void)//统一交由CubeMX配置
{
...
*********************************************************************************************************
*	函 数 名: sf_SetCS(0)
*	功能说明: 设置CS。 用于运行中SPI共享。共享要检查该spi总线是否在忙(不在该函数检查),是则退出。
*	形    参: 无
	返 回 值: 无
*********************************************************************************************************
*/
...
/*
*********************************************************************************************************
*	函 数 名: sf_SectorWrite(写入扇区,仅写入动作,不带擦除等)
*	功能说明: 向一个page内写入若干字节。字节个数不能超出页面大小（4K)
*	形    参:  	_pBuf : 数据源缓冲区；
*				_uiWriteAddr ：目标区域首地址
*				_usSize ：数据个数，不能超过SectorSize大小(如果起始地址偏移扇区起始地址,则能写入的个数为SectorSize-offset)
*	返 回 值: 无
*********************************************************************************************************
*/
void sf_SectorWrite(uint8_t * _pBuf, uint32_t _uiWriteAddr, uint16_t _usSize)
{
	uint32_t i, j;
...
*            目前采用02h命令,华邦的SPI FLASH限制50MHz,08h命令可以达到更高时钟,但时序不一样,要多发一个字节
*	形    参:  	_pBuf : 数据源缓冲区；
*				_uiReadAddr ：首地址
*				_usSize ：数据个数, 可以大于SectorSize,但是不能超出芯片总容量
*	返 回 值: 无
*********************************************************************************************************
*/
...
*	形    参:  	_ucpTar : 数据缓冲区
*				_uiSrcAddr ：Flash地址
*				_uiSize ：数据个数, 可以大于SectorSize,但是不能超出芯片总容量
*	返 回 值: 0 = 相等, 1 = 不等
*********************************************************************************************************
*/
static uint8_t sf_CmpData(uint32_t _uiSrcAddr, uint8_t *_ucpTar, uint32_t _uiSize)
...
*	功能说明: 写1个Sector并校验,如果不正确则再重写两次。注意,不能跨扇区写入。本函数自动完成擦除操作。
*	形    参:  	_pBuf : 数据源缓冲区；
*			   _uiWriteAddr ：目标区域首地址
*			   _usSize ：数据个数，不能超过SectorSize大小(如果起始地址偏移扇区起始地址,则能写入的个数为SectorSize-offset)
*	返 回 值: 0 : 错误， 1 ： 成功
...
static uint8_t sf_AutoWritePage(uint8_t *_ucpSrc, uint32_t _uiWrAddr, uint16_t _usWrLen)
{
	...
	for (i = 0; i < 3; i++)
	{
		...
		/* 编程一个扇区 */
		sf_SectorWrite(s_spiBuf, uiFirstAddr, g_tSF.SectorSize);
...
uint8_t sf_WriteBuffer(uint8_t* _pBuf, uint32_t _uiWriteAddr, uint16_t _usWriteSize)
{
	...
	else  /* 起始地址不是页面首地址  */
	{
		if (NumOfPage == 0) /* 数据长度小于页面大小 */
		{
			if (NumOfSingle > count) /* (_usWriteSize + _uiWriteAddr) > SPI_FLASH_SectorSize */
			{
...
uint32_t sf_ReadID(void)
{
	...
	id1 = bsp_spiRead1();					/* 读ID的第1个字节Manufacturer ID */
	id2 = bsp_spiRead1();					/* 读ID的第2个字节Memory Type ID15-8 */
	id3 = bsp_spiRead1();					/* 读ID的第3个字节Capacity ID7-0 */
	sf_SetCS(1);							/* 禁能片选 */
	...
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			case SST25VF016B_ID:
				...
				g_tSF.TotalSize = 2 * 1024 * 1024;	    /* 总容量 = 2MB */
				g_tSF.SectorSize = 4 * 1024;			/* 页面大小 = 4KB */
				break;
            ...
            case W25Q32_ID:
				...
				g_tSF.TotalSize = 8 * 1024 * 1024;	    /* 总容量 = 8MB */
				g_tSF.SectorSize = 4 * 1024;			/* 页面大小 = 4KB */
				break;
...
/*
*********************************************************************************************************
*	函 数 名: sf_GetCapacity
*	功能说明: 获取SPI FLASH的容量大小
*	形    参:  无
*	返 回 值: 无
*********************************************************************************************************
*/
uint32_t sf_GetCapacity(void)
{
	return (uint32_t)(g_tSF.TotalSize/g_tSF.SectorSize);
}
